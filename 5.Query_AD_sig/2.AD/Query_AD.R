...
##
## Dataset: Meta analysis AD - temporal cortex
##
##
...
# working directory
work_dir="/media/hamel/Workspace/Dropbox/Projects/CMap2/5.AD_data/5.2.AD/"

# directory of DE results of disease
data_dir="/media/hamel/Workspace/Dropbox/Projects/CMap2/5.AD_data/5.2.AD/"
...
disease_DE<-read.csv("Temporal_lobe_sig_AW_results_with_logFC.txt", sep="\t")

head(disease_DE)

#format into limma DE results
rownames(disease_DE)<-disease_DE$Entrez_Gene_ID
disease_DE$Entrez_Gene_ID<-NULL
colnames(disease_DE)<-c("logFC", "adj.P.Val")
...
disease_DE$CI.L<-0
disease_DE$CI.R<-0
disease_DE$AveExpr<-0
disease_DE$t<-0
disease_DE$P.Value<-0
disease_DE$B<-0
...
disease_DE<-disease_DE[c(1,3,5,6,7,8,2,4)]
...
as.data.frame(table(c(as.character(subset(method3_results, method3_results$P_Val<0.05)[,1]), as.character(subset(method5_results, method5_results$P_value<0.05)[,1]))))
