...
# EXPRESSION CHIP - HT_HG-U133A 
# CELL LINE - MCF7
# NUMBER OF SAMPLES - 2740 + 409 (controls)
...
# create dir
dir.create(paste(work_dir, paste("3.", data_name, "_processing", sep=""), sep="/"))
processing_dir=(paste(work_dir, paste("3.", data_name, "_processing", sep=""), sep="/"))
...
#create a dataset where probes >50th percentile in more than 80% of drugs + controls

# separate dataframe into case and control
exprs_data_sva_drug<-exprs_data_sva[exprs_data_sva$group=="Drug",]
exprs_data_sva_control<-exprs_data_sva[exprs_data_sva$group=="Control",]

dim(exprs_data_sva_drug)
dim(exprs_data_sva_control)

# run sample network on full data

#exprs_data_sva_drug_QC<-run_sample_network_plot(exprs_data_sva_drug, -10)
exprs_data_sva_drug_QC<-exprs_data_sva_drug
...
# Convert to a list
hgu133a.db_mapping <- as.data.frame(hthgu133aENTREZID[mapped_probes])
...
head(full_DE_results[[1091]])

dim(subset(full_DE_results[[1091]], adj.P.Val<=0.05))
...
dim(exprs_data_sva_QC_entrez_id_unique)
dim(number_of_DEG_in_full_data)
head(number_of_DEG_in_full_data)
tail(number_of_DEG_in_full_data)

summary(number_of_DEG_in_full_data$sig_DEG_count)
range(number_of_DEG_in_full_data$sig_DEG_count)
mean(number_of_DEG_in_full_data$sig_DEG_count)



# write DE result summary
setwd(DE_dir)
write.csv(number_of_DEG_in_full_data, file="DEG_count.txt", quote=F, row.names=F)
